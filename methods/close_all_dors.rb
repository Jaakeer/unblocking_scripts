class DirectOrderServices::CancelDirectOrder < BaseService
    ...
    @dpirs.each do |dpir|
      if dpir.dispatch_plan_status == "open"
        ...
        when "warehouse_to_buyer"
          dpir.pick_generated ? update_order_item_dp_prodution(dpir.order_item_id, dpir.shipped_quantity, dpir.dispatch_plan_id, "Cannot be rolled back as pick is created") : update_order_item_dp_planning(dpir.order_item_id, dpir.shipped_quantity, dpir.dispatch_plan_id)
  ...
  def update_no_dp_quantity
    @direct_order.order_items.each do |order_item|
      ...
      @order_item_cancelled = false
      no_dp_quantity = find_non_prod_plan_quantity(order_item, order_item.product_quantity)
  ...
  def check_open_pos
    @direct_order.order_items.each do |order_item|
      ...
      bizongo_po_items.each do |bizongo_po_item|
        if !dp_exists?(bizongo_po_item) && bizongo_po_item.bizongo_purchase_order.status != "cancelled"
  ...
  direct_order_ids.each do |dor_id|
    ...
    begin
      ...
      if service.errors.present?
        ...
        result.each do |k, ref|
          sum += ref["total_rollback"].to_i
        end
        adjusted_dors[dor_id] = sum
      end
    rescue => e
